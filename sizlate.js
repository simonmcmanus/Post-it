...
exports.doRender = function(str, options) {
	...
	var browser = require("jsdom/lib/browser");
	var dom = browser.browserAugmentation(require("jsdom/lib/level2/core").dom.level2.core);
	...
	var selectors = (typeof selectors[0] == 'undefined') ? [selectors] : selectors; // make sure we have an 
array. 
...
//			break;
//		}
		var a = (selectors[key].constructor == Array) ? selectors[key] : [selectors[key]]; // make sure we 
have an array.
		var c = a.length;
		var pendingItems = [];
		while(c--) {
...
//	console.log('selectors: ', selectors);
	for(key in selectors) {
		if(typeof selectors[key].partial !=="undefined" ){// this is a partial.	
			if(typeof selectors[key].data === "undefined" || selectors[key].data.length > 0){ // make 
sure we are passed in data and that the data is not empty.
				selectors[key] = 
exports.doRender('<body>'+exports.partials[selectors[key].partial]+'</body>', 
classifyKeys(selectors[key].data)).slice(6, -7);	// adding and then stripping body tag for jsdom. 				
			}
